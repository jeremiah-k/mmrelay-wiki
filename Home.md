...
Let's create a simple **HelloWorld** plugin to get started. This plugin will log "Hello world" when it receives a message from either Meshtastic or Matrix.
...
For this example, create a file named `hello_world.py` in your project repository.

### Step 2: Create the Plugin Class

Every plugin must inherit from `BasePlugin` and set its unique `plugin_name`. Here's the complete code for the HelloWorld plugin:
...
After adding the plugin to your configuration, restart the relay. If everything is set up correctly, you should see a line in your log indicating that the **HelloWorld** plugin has started:
...
Here's a simple plugin example that responds to specific commands without handling channels or DMs:
...
This example avoids complexities of channel and DM handling, making it easier for beginners. It uses the `bot_command` function from `matrix_utils.py` to check if the message is a command directed at the bot.
...
To make your plugin respond to specific commands in Matrix rooms, use the `bot_command` function from `matrix_utils.py`. This function helps determine if a message is a command directed at the bot.
